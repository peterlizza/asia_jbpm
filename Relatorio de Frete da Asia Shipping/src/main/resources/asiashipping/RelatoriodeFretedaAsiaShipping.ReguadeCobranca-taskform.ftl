<script type='text/javascript'>
	...
	function taskFormValidator() {
		...
		var myInputs = new Array();
					...
					i++;
					myInputs[i] = document.getElementById("dias_de_atraso");
					i++;
					myInputs[i] = document.getElementById("comentarios");
					i++;
		...
		var j=0;
					...
					j++;
						if(notEmpty(myInputs[j]) && !isNumeric(myInputs[j])) {
							alert("Please enter valid dias_de_atraso");
							myInputs[j].focus();
							return false;
						}
					j++;
						if(notEmpty(myInputs[j]) && !isAlphanumeric(myInputs[j])) {
							alert("Please enter valid comentarios");
							myInputs[j].focus();
							return false;
						}
					j++;
...
<div id="container">
	<div id="header">
		New Process Instance: /Relatorio de Frete da Asia Shipping/src/main/resources/asiashipping.Regua de Cobranca
	</div>
	<div id="content">
		...
		<fieldset>
            <legend>Process inputs</legend>
                            		...
                            		<label for="name">dias_de_atraso</label>
                            		<div class="div_texbox">
                              		<input name="dias_de_atraso" type="text" class="textbox" id="dias_de_atraso" value="" />
                            		</div>
              	
                            		<label for="name">comentarios</label>
                            		<div class="div_texbox">
                              		<input name="comentarios" type="text" class="textbox" id="comentarios" value="" />
                            		</div>
